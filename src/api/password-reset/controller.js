...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your Rest Mongo Boiler Plate account.<br>
        ...
        &mdash; Rest Mongo Boiler Plate Team
      `
      return sendMail({ toEmail: email, subject: 'Rest Mongo Boiler Plate - Password Reset', content })
    })
